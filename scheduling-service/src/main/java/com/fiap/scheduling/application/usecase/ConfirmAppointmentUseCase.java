...
/**
 * Confirma uma consulta (SCHEDULED -> CONFIRMED). Apenas DOCTOR e NURSE.
...
public class ConfirmAppointmentUseCase {
    ...
    public ConfirmAppointmentUseCase(AppointmentGateway appointmentGateway, EventPublisher eventPublisher) {
    ...
    public Appointment execute(UUID appointmentId, UUID currentUserId, UserRole currentUserRole) {
        if (currentUserRole != UserRole.ROLE_DOCTOR && currentUserRole != UserRole.ROLE_NURSE) {
            throw new AccessDeniedException("Apenas médicos e enfermeiros podem confirmar consultas");
        }
        ...
        if (currentUserRole == UserRole.ROLE_DOCTOR && !appointment.getDoctorId().equals(currentUserId)) {
            throw new AccessDeniedException("Médico só pode confirmar suas consultas");
        }
